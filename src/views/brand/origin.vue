<template>
  <div class="content_box">
    <m-title title='原产地管理'></m-title>
    <div class="searchBox">
      <el-row>
        <el-col :span='4'>
          <span class="timed">原产地名称：</span>
          <el-input v-model='search.OriginName' class="search_title" size='small' placeholder="请输入原产地名称">
          </el-input>
        </el-col>
        <el-col :span='4'>
    ...
    <el-table :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column label='原产地名称-中文' prop='NameCN' align='center'></el-table-column>
      <el-table-column label='原产地名称-英文' prop='NameEN' align='center'></el-table-column>
      ...
      <el-table-column label='操作' align='center' width='220'>
        <template slot-scope="scope">
          <el-button v-if='scope.row.InUse === 0 && checkBtn("func_common_disabled")' class="pan-btn green-btn" type="success" @click="editOrigin(scope.row)" size="mini" icon="el-icon-check">启用</el-button>
...
<script>
  ...
  import { getOriginList, editOrigin } from '@/api/brand/origin'
  export default {
    components: { MTitle },
    data() {
      return {
    ...
    methods: {
      ...
      getData() {
        getOriginList({ pageSize: this.pageSize, pageIndex: this.pageIndex, OriginName: this.search.OriginName, InUse: this.search.InUse }).then(res => {
