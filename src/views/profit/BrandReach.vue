<template>
  <div class="dashboard-container">
    <div class="bd">
      ...
      value-format='yyyy-MM'
      placeholder="选择结束月">
    </el-date-picker><span style="margin-left:10px;"></span>品牌：
      <el-select
        class="search_title"
        filterable
        clearable
...
import { exportBrandReach, getBrandReach, exportBrandReachData } from '@/api/charts/index'
export default {
  data() {
    return {
      chart: null,
  ...
  methods: {
    exportData() {
      ...
      exportBrandReachData({
        brandCodes: this.brandCode.join(','),
    ...
    getData() {
      ...
      exportBrandReach({
        ...
        a.BrandNameDate = []
        res.map(v => {
          a.BrandName.push(v.BrandName)
          a.BrandReach.push(v.BrandReach)
          ...
          a.BrandNameDate.push(str + '-' + v.BrandName)
        })
        this.initChart(a)
      })
    },
    getBrand(cb) {
      getBrandReach().then(res => {
        this.brandList = res;
        res.map(v => {
          this.brandCode.push(v.Code)
        })
        this.$nextTick(() => {
          cb()
        })
